...
def psnr(img1, img2, mask=None):
    ...
    if mask is not None:
        if mask.dim() == 2:
            mask = mask.unsqueeze(0).expand_as(diff_sq)
        valid = (mask > 1e-6).float().expand_as(diff_sq)
        if valid.sum() < 1:
            return torch.tensor(0.0, device=img1.device, dtype=img1.dtype)
        mse = (diff_sq * valid).sum() / (valid.sum() + 1e-8)
        return 20 * torch.log10(1.0 / (torch.sqrt(mse) + 1e-8))
    mse = diff_sq.view(img1.shape[0], -1).mean(1, keepdim=True)
    return 20 * torch.log10(1.0 / (torch.sqrt(mse) + 1e-8))
