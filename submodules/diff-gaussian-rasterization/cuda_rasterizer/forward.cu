...
/* OmniGS: 2D covariance for equirectangular / LonLat (GPL) */
	...
	float* __restrict__ out_invdepth)
{
	...
	bool compute_metric = (metric_map != nullptr && metric_count != nullptr && inside);
	if (compute_metric)
		pixel_error = metric_map[pix_id];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (out_invdepth)
			{
				float zcam = collected_depth[j];
				float invd = 1.f / fmaxf(zcam, 1e-6f);
				expected_invdepth += invd * alpha * T;
			}

			// ===== 累加误差贡献 =====
			if (compute_metric && pixel_error > 0.0f)
			{
				atomicAdd(&metric_count[coll_id], alpha * T * pixel_error);
			}
...
/* OmniGS: LonLat preprocess kernel + host wrapper (GPL) */
template<int C>
__global__ void preprocessLonlatCUDA(int P, int D, int M,
	const float* orig_points,
	const glm::vec3* scales,
	const float scale_modifier,
	const glm::vec4* rotations,
	...
	bool prefiltered)
{
	...
	if (too_close(p_orig, viewmatrix, p_view))
		return;
	float2 p_proj = point3ToLonlatScreen(p_view);
	const float* cov3D;
	if (cov3D_precomp != nullptr)
		cov3D = cov3D_precomp + idx * 6;
	else
