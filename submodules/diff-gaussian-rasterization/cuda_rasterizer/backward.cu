...
/* OmniGS: LonLat inverse 2D cov backward (GPL) */
	...
	float3* dpy_dt)
{
	...
	float dL_dtx = -dL_dJ00 * temp4 + dL_dJ02 * temp3
		+ dL_dJ10 * temp5 * (2.0f * txtx * trxztrxz - tztz * trtr)
		+ dL_dJ11 * t.x * temp1 + dL_dJ12 * temp2;
	float dL_dty = dL_dJ10 * t.x * temp1 - dL_dJ11 * H_div_pi * 2.0f * trxz * t.y * trtrtrtr_inv + dL_dJ12 * t.z * temp1;
	float dL_dtz = dL_dJ00 * temp3 + dL_dJ02 * temp4 + dL_dJ10 * temp2 + dL_dJ11 * t.z * temp1
		+ dL_dJ12 * temp5 * (2.0f * tztz * trxztrxz - txtx * trtr);
	if (dL_dinvdepth)
	{
		// d(1/r)/d(tx,ty,tz) = -(tx,ty,tz) / r^3
		float r_sq = trtr;
		float r = sqrtf(r_sq);
		float r3_inv = 1.0f / (r * r_sq + 1e-7f);
		float grad = dL_dinvdepth[idx];
		dL_dtx -= grad * t.x * r3_inv;
		dL_dty -= grad * t.y * r3_inv;
		dL_dtz -= grad * t.z * r3_inv;
	}
	float3 dL_dmean = transformVec4x3Transpose({ dL_dtx, dL_dty, dL_dtz }, view_matrix);
	dL_dmeans[idx] = dL_dmean;
}
...
__global__ void preprocessCUDA(
	...
	// [AbsGS] only .x/.y are consumed for back-prop; .z/.w hold absolute gradients used for densification
...
/* OmniGS: LonLat preprocess backward (GPL) */
template<int C>
...
renderCUDA(
	...
	const float* __restrict__ dL_dout_invdepth,
	const float* __restrict__ depths,
	float* __restrict__ dL_dinvdepth_per_gauss,
	// [AbsGS] dL_dmean2D layout: (dx, dy, |dx|, |dy|) per Gaussian.
	...
	float* __restrict__ dL_dcolors)
{
	...
	if (inside)
	{
		...
		if (dL_dout_invdepth)
			dL_invdepth = dL_dout_invdepth[pix_id];
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (dL_dout_invdepth && dL_dinvdepth_per_gauss)
			{
				const float invd = 1.f / fmaxf(collected_depth[j], 1e-6f);
				...
				atomicAdd(&(dL_dinvdepth_per_gauss[global_id]), dchannel_dcolor * dL_invdepth);
			}
			dL_dalpha *= T;
			...
			atomicAdd(&dL_dmean2D[global_id].x, dmx);
			atomicAdd(&dL_dmean2D[global_id].y, dmy);
			atomicAdd(&dL_dmean2D[global_id].z, fabsf(dmx));
			atomicAdd(&dL_dmean2D[global_id].w, fabsf(dmy));
...
void BACKWARD::preprocess(
	...
	const float tan_fovx, float tan_fovy,
	const glm::vec3* campos,
	const float* dL_dinvdepth_per_gauss,
	// [AbsGS] only .x/.y are used here; .z/.w stay in the tensor for densification statistics
	const float4* dL_dmean2D,
...
void BACKWARD::render(
	...
	const float* dL_dout_invdepth,
	const float* depths,
	float* dL_dinvdepth_per_gauss,
	// [AbsGS] dL_dmean2D layout: (dx, dy, |dx|, |dy|) per Gaussian.
	float4* dL_dmean2D,
	float4* dL_dconic2D,
	float* dL_dopacity,
