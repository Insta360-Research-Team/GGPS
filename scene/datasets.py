...
class GSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        invdepthmap = viewpoint_cam.invdepthmap
        depth_mask = viewpoint_cam.depth_mask
        depth_reliable = viewpoint_cam.depth_reliable
        sky_mask = getattr(viewpoint_cam, "sky_mask", None)
        ##保存成bool格式
        if mask is not None:
            mask = (mask > 1e-6)
        if depth_mask is not None:
            depth_mask = (depth_mask > 1e-6)
        return x, y, mask, invdepthmap, depth_mask, depth_reliable, sky_mask
